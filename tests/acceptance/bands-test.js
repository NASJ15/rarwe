...
module('Acceptance | Bands', function(hooks) {
  ...
  test('Sort songs in various ways', async function(assert) {
	...
	assert.equal(currentURL(), '/bands/1/songs?s=titleDesc', 'OK');

	await click('[data-test-rr=sort-by-rating-asc]');
	assert.dom('[data-test-rr=song-list-item]:first-child').hasText('Mind Eraser, No Chaser', 'The first song is the highest ranked,first one in the alphabet');
	assert.dom('[data-test-rr=song-list-item]:last-child').hasText('Spinning In Daffodils', 'The last song is the lowest ranked, last one in the alphabet');
	assert.equal(currentURL(), '/bands/1/songs?s=ratingAsc', 'OK');
	...
	assert.equal(currentURL(), '/bands/1/songs?s=titleAsc', 'OK');
  });
  ...
  test('Search songs', async function(assert) {
	...
	assert.equal(currentURL(), '/bands/1/songs?q=no' ,'OK');
	...
	assert.ok(currentURL().includes('q=no'), 'OK');
	assert.ok(currentURL().includes('s=titleDesc'), 'OK');
  });
